...
typedef struct AtCmdEntityQueueTag
{
    ...
    u8 funFirst[2];                                /* 功能模块的第一条 */
    u8 funLast[2];                                 /* 功能模块的最后一条 */
    u8 funCount;
...
static ResultNotifyCb p_atQueueDefaultCb = NULL;
...
{
    u8 i = 0;
    u8 first = MIN(s_atCmdEntityQueue.current,s_atCmdEntityQueue.funFirst[0]);
...
{
                                                /* get first index */
    u8 first =  MAX(s_atCmdEntityQueue.current,s_atCmdEntityQueue.funFirst[0]);
...
//    if (s_atCmdEntityQueue.funLast != s_atCmdEntityQueue.funFirst){
    ...
    s_atCmdEntityQueue.funFirst[s_atCmdEntityQueue.funCount-1] = s_atCmdEntityQueue.last;
    s_atCmdEntityQueue.funLast[s_atCmdEntityQueue.funCount -1] = (s_atCmdEntityQueue.last + funCount - 1 + AT_CMD_QUEUE_COUNT) % AT_CMD_QUEUE_COUNT;
    eat_trace("INFO: the at cmd func's range is %d-%d", s_atCmdEntityQueue.funFirst[s_atCmdEntityQueue.funCount-1], s_atCmdEntityQueue.funLast[s_atCmdEntityQueue.funCount-1]);
    return TRUE;
}
/* 
 * ===  FUNCTION  ======================================================================
...
eat_bool simcom_atcmd_queue_fun_append(AtCmdEntity atCmdEntity[],u8 funCount)
{
    ...
    eat_trace("INFO: funFirst:%d  funLast:%d", s_atCmdEntityQueue.funFirst[s_atCmdEntityQueue.funCount-1], s_atCmdEntityQueue.funLast[s_atCmdEntityQueue.funCount-1]);
    return TRUE;
}
            ...
            case EAT_EVENT_TIMER :
                ...
                switch ( event.data.timer.timer_id ) {
                    ...
                    case EAT_TIMER_2:           /* at cmd time over */
                    	...
                    	AtCmdOverTimeStop();
						if(p_atQueueDefaultCb)
						{
							p_atQueueDefaultCb(EAT_FALSE);
							p_atQueueDefaultCb = NULL;
						}
...
static eat_bool simcom_atcmd_queue_head_out(void)
{
   ...
   if(s_atCmdEntityQueue.current>=s_atCmdEntityQueue.funLast[0]
           || s_atCmdEntityQueue.current<=s_atCmdEntityQueue.funFirst[0]){
...
static eat_bool simcom_atcmd_queue_fun_out(void)
{
    ...
    for ( i=s_atCmdEntityQueue.funFirst[0] ;i<=s_atCmdEntityQueue.funLast[0] ; i++) {
        AtCmdEntity* atCmdEnt = NULL;
        atCmdEnt = &(s_atCmdEntityQueue.atCmdEntityArray[i]);
        if(atCmdEnt->p_atCmdStr){
            eat_mem_free(atCmdEnt->p_atCmdStr);
    ...
    if(s_atCmdEntityQueue.current != s_atCmdEntityQueue.funLast[0]){
        ...
        s_atCmdEntityQueue.current = s_atCmdEntityQueue.funLast[0];
    }

    s_atCmdEntityQueue.current = (s_atCmdEntityQueue.current + 1) % AT_CMD_QUEUE_COUNT;
    if(s_atCmdEntityQueue.funCount==2){
        s_atCmdEntityQueue.funLast[0] = s_atCmdEntityQueue.funLast[1];
        s_atCmdEntityQueue.funFirst[0] = s_atCmdEntityQueue.funFirst[1];
        s_atCmdEntityQueue.funLast[1] = 0;
        s_atCmdEntityQueue.funFirst[1] = 0;
    }
    else {
        s_atCmdEntityQueue.funLast[0] = 0;
        s_atCmdEntityQueue.funFirst[0] = 0;
    }
    s_atCmdEntityQueue.funCount--;
    return TRUE;
            ...
            switch ( atCmdRsp ) {
                ...
                default:	
                    if(atCmdRsp>=AT_RSP_STEP_MIN && atCmdRsp<=AT_RSP_STEP_MAX) {
                        ...
                        if(step<=s_atCmdEntityQueue.funLast[0] && step>= s_atCmdEntityQueue.funFirst[0]){
...
void simcom_atcmd_queue_default_cb_set(ResultNotifyCb callback)
{
	p_atQueueDefaultCb = callback;
}
