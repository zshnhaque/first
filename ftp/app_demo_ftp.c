...
eat_bool eat_ftp_paras_set(u8 **paras, u8 **datas, u8 length, ResultNotifyCb syncCB)
{
	...
	//set fun size
	result = simcom_atcmd_queue_fun_set(length);
	if(result != TRUE)
	{
	...
	simcom_atcmd_queue_default_cb_set(eat_ftp_para_set_default_cb);
...
eat_bool eat_ftpput(u8 *pBufer, u32 bufSize, AtCmdRspCB sync)
{
	...
	result = simcom_atcmd_queue_fun_append(atCmdInit, 4);
...
eat_bool eat_ftpget(AtCmdRspCB sync, GetDataHandle dataHandle)
{
	...
	return simcom_atcmd_queue_fun_append(atCmdInit, 2);
}
